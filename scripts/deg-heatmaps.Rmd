...
upload individual RNAseq gene count data (2020 Genewiz samples):
```{r}
master_individual_counts <- read.delim("../analyses/individual-RNAseq-genecounts_log.txt", sep = '\t')
head(master_individual_counts)
```

Pull out column "X" (Trinity IDs) and all individual samples that have _log in column name: 
```{r}
...
```{r}
include_list <- c("TRINITY_DN0_c4_g1", "TRINITY_DN3868_c0_g1", "TRINITY_DN28066_c26_g1","TRINITY_DN1362_c0_g1")
ABC.heatmap.sub <- ABC.heatmap[include_list, ]
ABC.heatmap.sub
```

```{r}
pheatmap(ABC.heatmap.sub, cluster_row = TRUE, cluster_cols = FALSE, clustering_distance_rows = "euclidean", clustering_method = "average", show_rownames = FALSE, show_colnames = TRUE, treeheight_col = 80, legend = TRUE, color = heatmapPRGn, fontsize_col = 20, fontsize_row = 12)
``` 



I will annotate this list first and then re-do and just map those that have annotations. 
