...
```{r}
#write.table(abcghi.res.sig, "../analyses/DEGlist-abcghi-timeseries.tab", sep = "\t", row.names = T, quote = FALSE, col.names = TRUE)
```
...
```{r}
resultsNames(abcghi.dds)
```

```{r}
res2 <- results(abcghi.dds, name = "temperatureelevated.dayday2", test = "Wald")
res2[which.min(abcghi.res$pvalue),]
```

```{r}
betas <- coef(abcghi.dds)
colnames(betas)
```
...
mat[mat < -5] <- -5
mat[mat > 5] <- 5
topGenes <- head(order(abcghi.res$pvalue),10)
